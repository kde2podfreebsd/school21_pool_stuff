...
a - append (добавление)
r+ - read & write
w+ - read + wrtie + создание если его не существовало
a+ - read + apend
...
fopen -> oc ->file 
дескриптор файлового потока (буфер) (поэтому открываем и закрываем)

EOF эквивалент  \0 для строк

fseef(файл поток, оффсет относительно 3его параметра, )
seek_set - от начал поз
seek_cur - от текущей поз
seek_end - от конец поз

ftell - текущая файловая позиция (число байт)
*/
...
void encryptCesar(char *text, int key) {
    ...
    for (int i = 0; text[i] != '\0'; ++i) {
        // Проверка, является ли текущий символ печатаемым ASCII-символом
        if (text[i] >= 33 && text[i] <= 126) {
            // Формула шифрования Цезаря:
            // 1. Вычитаем 33, чтобы привести диапазон символов к 0-93.
            // 2. Добавляем ключ шифрования.
            // 3. Вычисляем остаток от деления на 94, чтобы вернуться в диапазон 0-93.
            // 4. Добавляем 33 обратно, чтобы вернуться к печатаемым ASCII-символам.
